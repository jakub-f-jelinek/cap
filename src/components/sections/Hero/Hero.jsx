...
export default function Hero() {
  ...
  const applyProgress = (progress) => {
    ...
    if (textboxRef.current) {
      textboxRef.current.style.transform = `translateX(${shrink * -18}%) scale(${1 - shrink * 0.42})`
    }

    const statReveal = Math.min(Math.max((progress - STAT_START) / (STAT_END - STAT_START), 0), 1)
    if (statRef.current) {
      ...
      <div className="hero__stage">
        <video
          ...
          onLoadedMetadata={(event) => {
            event.currentTarget.currentTime = progressRef.current * event.currentTarget.duration
          }}
        />
        ...
        <div className="hero__content">
          ...
          <div ref={statRef} className="hero__stat">
            ...
            <p className="hero__stat-copy">
              Chování, které při řízení považujeme za normální, si od 1. ledna 2026 vyžádalo 420 životů.
            </p>
          </div>
        </div>
      </div>
